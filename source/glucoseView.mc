...
class glucoseView extends WatchUi.DataField {
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        if (status == 1) {
            ...
            var statusColor = status == 2 ? Graphics.COLOR_YELLOW : Graphics.COLOR_RED;
    ...
    function drawGlucoseGraph(dc as Graphics.Dc, width as Number, height as Number) as Void {
        ...
        // Time range
        var minT = readings[0][:timestamp];
        var maxT = readings[readings.size() - 1][:timestamp];
